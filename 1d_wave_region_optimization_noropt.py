...
def write_results(name, seed, model, rl2, time, region):
    # PDE name, seed, model, relative L2, initial region
    with open("./results/results_noropt.csv", "a") as out:
        out.write(
            f"{name},{seed},{model},{rl2:4f},{int(time)},{region}\n"
        )
...
write_results("1dReactionRegion", args.seed, args.model, rl2, time.time()-START, initial_region)
